...
{
  if (GET_CHAR_SKILL(ch, SKILL_ADVANCED_MEDITATION)/10 > number(0,100)) {
...
void do_meditate(P_char ch, char *arg, int cmd)
{
  ...
  if (GET_CHAR_SKILL(ch, SKILL_ADVANCED_MEDITATION)) {
    ...
    af.modifier = 20 + GET_CHAR_SKILL(ch, SKILL_ADVANCED_MEDITATION)/2;
    af.duration = -1;
    affect_to_char(ch, &af);
    ...
    for (af = ch->affected, count = 1; af; af = af->next)
      if (af->type == TAG_KNOWN_SHAPE) {
        sprintf(buf, "[%d] %s &n(&+W%s&n)\n", count++, mob_index[real_mobile(af->modifier)].desc2,
            how_learned[(5 * af->duration - 1)/get_property("innate.shapechange.memory.time", 500)]);
        send_to_char(buf, ch);
      }
    return;
  }
